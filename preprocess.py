    ...
    with open(gamma2_filename) as gamma2:
        ...
        for row in gamma2_lines:
            ...
            if len(entry) == 4:
                ...
                data_gamma2.append(np.array([np.std(x_vals), np.std(y_vals), np.std(z_vals), np.std(e_vals), np.mean(x_vals), np.mean(y_vals), np.mean(z_vals), np.mean(e_vals), count, 1]))
                x_vals = []
                y_vals = []
                z_vals = []
                e_vals = []
    ...
    with open(neutron2_filename) as neutron2:
        ...
        for row in neutron2_lines:
            ...
            if len(entry) == 4:
                ...
                data_neutron2.append(np.array([np.std(x_vals), np.std(y_vals), np.std(z_vals), np.std(e_vals), np.mean(x_vals), np.mean(y_vals), np.mean(z_vals), np.mean(e_vals), count, 0]))
                x_vals = []
                y_vals = []
                z_vals = []
                e_vals = []
